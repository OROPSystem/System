...
# choose language
with open("system/language/text-zh.json", 'r', encoding="utf-8") as f:
    json_zh = json.load(f)
with open("system/language/text-en.json", 'r', encoding="utf-8") as f:
    json_en = json.load(f)
...
# TODO 做到数据库去，在网页require前读取language信息
@app.before_request
def before_request():
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not checklogin(
                form, login_user, session, sess, USER_ROOT, local_sess, appConfig
        ):
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        return render_template(
            "login.html", form=LoginForm(), token=get_token_user(username)
        )
    return render_template(
        "signup.html", form=form, user=username, token=get_token_user(username)
    )
...
def user_data():
    ...
    _, param_configs = config_ops.get_configs_files(
        USER_ROOT, username, not_validated=True
    )
    ...
    if form.validate_on_submit():
        try:
            dataset_name = config_ops.new_config(
                form.data["train_file"], form.data["test_file"], USER_ROOT, username
            )
...
def upload_image():
    ...
    if form.validate_on_submit():
        ...
        try:
            dataset_name = config_ops.new_image_dataset(
                USER_ROOT, username, option_selected, file
            )
            return jsonify(status="ok", msg=dataset_name)
        except Exception as e:
            return jsonify(status="error", msg=str(e))
...
def gui_load():
    ...
    if local_sess.load_config():
        ...
        return render_template(
            ...
            user_dataset=user_dataset,
            parameters=param_configs,
            dataset_params=hlp.get_dataset_params(),
            data=hlp.get_data(),
    ...
    return render_template(
        "gui.html",
        token=get_token_user(session["user"]),
        user=username,
        user_dataset=user_dataset,
        cy_model=cy_model,
        model_name=model_name,
...
def params_run():
    ...
    try:
        ...
        checkpoints = run_utils.get_eval_results(
        ...
        (
            ...
            graphs,
            log_mess,
        ) = run_utils.define_empty_run_params()
        return jsonify(
            checkpoints=checkpoints,
            parameters={},
            metric=metric,
...
def params_predict():
    try:
        local_sess = Session(app, appConfig)
        hlp = load_local_sess(
            local_sess, request, session["user"], session["_id"], USER_ROOT
        )
        export_dir = config_reader.read_config(
            local_sess.get_config_file()
        ).export_dir()
        checkpoints = run_utils.get_eval_results(
            export_dir, local_sess.get_writer(), local_sess.get_config_file()
        )
        metric = local_sess.get_metric()
        params, _ = hlp.get_default_data_example()
        has_test = hlp.has_split_test()
        return jsonify(
            checkpoints=checkpoints, metric=metric, params=params, has_test=has_test
        )
    except (KeyError, NoSectionError):
...
def run():
    ...
    (
        ...
        graphs,
        log_mess,
    ) = run_utils.load_run_config(sess, th, username, form, USER_ROOT, model_configs)
    ...
    return render_template(
        ...
        token=get_token_user(username),
        form=form,
        user_models=model_configs,
        dataset_params=user_datasets,
        running=running,
        model_name=model_name,
        checkpoints=checkpoints,
        metric=metric,
...
def predict():
    ...
    if request.method == "POST":
        ...
        return (
            jsonify(error=final_pred)
            if not success
            else jsonify(run_utils.get_predictions(hlp.get_targets(), final_pred))
        )
...
def test():
    ...
    if request.method == "POST":
        ...
        test_output = process_test_request(
        ...
        if (
                len(
                    [
                        ...
                        if os.path.isdir(os.path.join(export_dir, i))
                    ]
                )
                == 0
        ):
...
def delete_dataset():
    username = session["user"]
    sys_ops.delete_dataset(
        get_all(request), get_dataset(request), get_models(request), username, USER_ROOT
    )
    _, models = config_ops.get_configs_files(USER_ROOT, username)
    datasets = config_ops.get_datasets(USER_ROOT, username)
    return jsonify(
...
def refresh():
    ...
    try:
        hlp = sess.get_helper()
        all_params_config = config_reader.read_config(sess.get_config_file())
        epochs = run_utils.get_step(
            hlp.get_train_size(),
            all_params_config.train_batch_size(),
...
def default_prediction():
    ...
    all_params_config = run_utils.create_result_parameters(
        request, local_sess, checkpoint=checkpoint
    )
    new_features = hlp.get_new_features(None, default_features=True)
    sys_ops.get_canned_data(
        USER_ROOT, username, get_modelname(request), all_params_config
    )
...
def deploy():
    ...
    if request.method == "POST" and "model_name" in request.form:
        ...
        file_path = sys_ops.export_models(
            all_params_config.export_dir(), request.form["selected_rows"], model_name
        )
        return send_file(
            file_path,
            mimetype="application/zip",
            attachment_filename=ntpath.basename(file_path),
            as_attachment=True,
        )
    _, param_configs = config_ops.get_configs_files(USER_ROOT, username)
    return render_template(
...
def notfound(e):
    return render_template(
        "error.html", error=e.name, number=e.code, message=e.description
    )
